...
In the first plot, the embeddings generated by the word-only BiLSTM are loosely clustered and partially overlapped.
Several entity categories (PER, LOC, ORG, MISC) do not form clearly separated regions:

- The “O” class dominates the space and spreads widely.

- PER, ORG, and MISC tokens appear intermixed.

- Boundaries between entity classes are blurry.

This indicates that the baseline model learns weakly discriminative token representations.
Because it relies solely on word embeddings and independent softmax predictions, it struggles to capture: 

- morphological patterns 

- capitalization signals 

- structural relationships between entity tags 

and therefore produces more inconsistent or inaccurate predictions.

#### Proposed Model (Word + Char + CRF)

![PCA CRF](./figure/token_pca_bilstm_crf_word_char.png)

In contrast, the second PCA plot shows significantly more structured and well-separated clusters:

- PER, LOC, ORG, and MISC categories form distinct, coherent groups.

- Within‐class variance is reduced (clusters are tighter).

- Between-class boundaries are clearer, with much less overlap.

- The embedding space exhibits a more organized geometric structure.

This demonstrates that adding character-level features provides richer morphological information (e.g., capitalization, suffixes, prefixes), and the CRF layer helps enforce consistent label dependencies (BIO transitions).
As a result, the model learns semantically meaningful and more discriminative representations for each token.

#### Overall Interpretation

The comparison between the two PCA plots visually confirms the improvement brought by the enhanced model:

- The baseline produces diffuse and overlapping embeddings → weaker representation learning.

- The Word+Char+CRF model produces clearly separated clusters → stronger entity discrimination.

These visualizations directly support the higher dev/test accuracy of the CRF model and justify the architectural choices made in the project.
